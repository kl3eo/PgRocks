...
my $c = `/usr/local/pgsql/bin/psql -c "\\timing" -c "drop table if exists $tab\_c0" people > .err0.log 2>&1`;

system("echo ==== >> log && date >> log && echo 'Creating empty $tab\_c0' >> log && echo ==== && date && echo 'Creating empty $tab\_c0'");

my $c = `/usr/local/pgsql/bin/psql -c "\\timing" -c "select _e_checkout_c0('$tab',0)" people >> .err0.log 2>&1`;

$c = `/usr/local/pgsql/bin/psql -c "\\timing" -c "alter table $tab\_c0 set (paraller_workers=20)" people >> .err0.log 2>&1`;
} else {
my $c = `/usr/local/pgsql/bin/psql -c "\\timing" -c "DROP TRIGGER IF EXISTS $tab\_c0_biud_v3 ON $tab\_c0" people >> .err0.log 2>&1`;
}
...
for (my $i = $ini; $i <$U-1; $i++) {
	my $j = $i + 1;
	my $c = `/usr/local/pgsql/bin/psql -c "\\timing" -c "select _e_checkout_c0('$tab',$j)" people > .err$j.log 2>&1 &`	
...
my $c = `/usr/local/pgsql/bin/psql -c "\\timing" -c "select _e_checkout_c0('$tab',$U)" people > .err$U.log 2>&1`;
...
my $k = int(rand(15)) + 1;
$c = `/usr/local/pgsql/bin/psql -c "\\timing" -c "DROP TRIGGER IF EXISTS $tab\_c0_biud_v3 ON $tab\_c0" people >> .err0.log 2>&1`;
$c = `/usr/local/pgsql/bin/psql -c "\\timing" -c "CREATE TRIGGER $tab\_c0_biud_v3 BEFORE INSERT OR UPDATE OR DELETE  ON $tab\_c0 FOR EACH ROW EXECUTE PROCEDURE _ew_new_row('$tab',$k)" people >> .err0.log 2>&1`;
