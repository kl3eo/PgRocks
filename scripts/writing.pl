...
if ($num_args != 6) {
    ...
    print "\nUsage: '$0 <table> <port> <DATABASE> <number of ROWS> <initial RocksDB store number: default 1> <number of writing streams: default 16>'\n";
    exit;
}
...
my $N = defined($ARGV[5]) && int($ARGV[5]) > 0 ? int($ARGV[5]) : 16;
...
my $c = `/usr/local/pgsql/bin/psql -p $port -c "\\timing" -c "select _i_v3_dna('$tab',1)" $dbase > .err0.log 2>&1`;


system("echo ==== >> log && date >> log && echo '(Re)creating clone $tab\_new' >> log && echo ==== && date && echo '(Re)creating clone $tab\_new'");
$c = `/usr/local/pgsql/bin/psql -p $port -c "\\timing" -c "drop table if exists $tab\_new" $dbase >> .err0.log 2>&1`;
$c = `/usr/local/pgsql/bin/psql -p $port -c "\\timing" -c "create unlogged table $tab\_new as select * from $tab limit 0" $dbase >> .err0.log 2>&1`;
$c = `/usr/local/pgsql/bin/psql -p $port -c "\\timing" -c "alter table $tab\_new add column my_$tab\_new_id bigserial" $dbase >> .err0.log 2>&1`;
...
$c = `/usr/local/pgsql/bin/psql -p $port -c "\\timing" -c "insert into $tab\_new select * from $tab" $dbase >> .err0.log 2>&1`;
system("echo ==== >> log && date >> log && echo 'Indexing clone $tab\_new' >> log && echo ==== && date && echo 'Indexing clone $tab\_new'");
$c = `/usr/local/pgsql/bin/psql -p $port -c "\\timing" -c "alter table $tab\_new add primary key (my_$tab\_new_id)" $dbase >> .err0.log 2>&1`;
...
for (my $i = $ini; $i <$nru; $i++) {
	...
	my $c = `/usr/local/pgsql/bin/psql -p $port -c "\\timing" -c "explain analyze select _e_v3_dna('$tab',$j, $lim,$sum,'asc')" $dbase > .err$j.log 2>&1 &`;
print "Here store is $j, rows is $lim, offset is $sum, order is asc!\n";	
	$c = `/usr/local/pgsql/bin/psql -p $port -c "\\timing" -c "explain analyze select _e_v3_dna('$tab',$k, $lim,$sum,'desc')" $dbase > .err_bis_$j.log 2>&1 &`;
...
my $c = `/usr/local/pgsql/bin/psql -p $port -c "\\timing" -c "explain analyze select _e_v3_dna('$tab',$k,$lim,$sum,'desc')" $dbase > .err_bis_$j.log 2>&1 &`;
print "Here store is $k, rows is $lim, offset is $sum, order is desc!\n";

$lim = $nrows - 2*$sum - $lim;

print "Here store is $j, rows is $lim, offset is $sum, order is asc!\n";
$c = `/usr/local/pgsql/bin/psql -p $port -c "\\timing" -c "explain analyze select _e_v3_dna('$tab',$j,$lim,$sum,'asc')" $dbase > .err$j.log 2>&1`;
...
$c = `/usr/local/pgsql/bin/psql -p $port -c "\\timing" -c "create index $tab\_v3_dna_idx on $tab\_v3_dna (key,mark)" $dbase >> .err0.log 2>&1`;
