...
function update_table_in_joint(name, from, tableNum, jsonRow) {
  ...
  client.query('UPDATE v2_dna_' + tableNum + ' SET rev = ($2) WHERE key = ($3) and tab = ($1)'
  ...
  client.query('DELETE from ' + row.tab + '_c0_' + tableNum + ' WHERE key = ($1)'
    , [row.key])
