...
/*
PgRocks, E.Gurianov, A.Shevlakov, 2018.
*/

/*
 * Turn a Datum into csv text, appending the string to "result".
 *
 * tcategory and outfuncoid are from a previous call to json_categorize_type,
 * except that if is_null is true then they can be invalid.
 *
...
row_to_json_rocks(PG_FUNCTION_ARGS)
{
	...
	if (err != NULL) {
		ereport(ERROR,
				...
				errmsg("[rocksdb], put error: %s", err)));
	}
...
extern Datum
row_to_json_rocks_sst(PG_FUNCTION_ARGS)
{
