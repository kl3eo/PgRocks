...
static Datum populate_composite_old(CompositeIOData *io, Oid typid, int32 typmod,
				   const char *colname, MemoryContext mcxt,
...
/*
PgRocks, E.Gurianov, A.Shevlakov, 2017-18.
*/
static Datum
populate_composite_old(CompositeIOData *io,
				   Oid typid,
				   int32 typmod,
				   const char *colname,
				   MemoryContext mcxt,
...
//isnull = JsValueIsNull(&jsv);
	...
	rettuple = populate_composite_old(&cache->c.io.composite, tupType, tupTypmod, NULL, fnmcxt, rec, &jsv);
//rettuple = populate_composite(&cache->c.io.composite, cache->argtype, NULL, fnmcxt, rec, &jsv, isnull);

	if (tupdesc)
	{
		cache->c.io.composite.tupdesc = NULL;
		ReleaseTupleDesc(tupdesc);
	}
				...
				char* csvLine, int lineLen)
{
	...
	/*
	 * if the input csv is empty, we can only skip the rest if we were passed
	 * in a non-null record, since otherwise there may be issues with domain
	 * nulls.
	 */
	if (defaultval && lineLen == 0)
		return defaultval;
	...
	for (i = 0; i < ncolumns; ++i)
	{
		...
		found = _csv_get_next_field(csvLine, lineLen, &csv_parser_pos, &field);
...
/* recursively populate a composite (row type) value from csv */
static Datum
_csv_populate_composite(CompositeIOData *io,
				   Oid typid,
				   int32 typmod,
				   const char *colname,
				   MemoryContext mcxt,
				   ...
				   char* csvLine, int lineLen)
{
	...
	/* populate resulting record tuple */
	tuple = _csv_populate_record(io->tupdesc, &io->record_io,
							defaultval, mcxt, csvLine, lineLen);
...
rocks_csv_populate_record_worker(FunctionCallInfo fcinfo)
{
	...
	rettuple = _csv_populate_composite(&cache->c.io.composite, tupType, tupTypmod, NULL, fnmcxt, rec, rocks_value_buf, csvLineLen);
