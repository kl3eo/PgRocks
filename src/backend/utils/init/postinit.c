...
void _rocksdb_close()
{
	// finish batch records
	char *err = 0;
	if (writebatch_records != 0) {
	...
	if (rocksdb) rocksdb_close(rocksdb);	
 ...
 * descriptors for both pg_database and its indexewritebatchs from the shared relcache
