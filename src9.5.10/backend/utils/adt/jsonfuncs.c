...
rocks_json_populate_record_worker(FunctionCallInfo fcinfo)
{
	...
	//fprintf(stderr, "[rocksdb]: buf %s\n", buf);

	json = (text *) palloc(len + VARHDRSZ);
	SET_VARSIZE(json, len + VARHDRSZ);
	memcpy((void *) VARDATA(json),
		   (void *)returned_value,
		   len);

	json_hash = get_json_object_as_hash(json, "rocks_json_populate_record_worker");
